...
This is called a **convolution**. It's essentially a combination of two functions by sliding one function (in this case <Latex formula='w(a)' />) over another (<Latex formula='x(t)' />), multiplying then together and and integrating the result.

<h5>Discretized Convolution</h5>

It's not realistic for our pressure sensor to make a reading at every possible timestep <Latex formula='t' />. Instead, we're probably going to be taking readings at **discrete** time steps. For example, we might take a reading ever 10 seconds.

This is similar to the continous convolution above, only the integral becomes a sum:
